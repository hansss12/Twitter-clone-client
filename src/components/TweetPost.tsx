...
import { threadPost } from "../store/actions/actions";
...
const TweetPost: React.FC = () => {
  const dispatch = useDispatch()
    ...
    const postTweet = async (event: any) => {
      ...
      const response = await dispatch(threadPost(tweet))
      if (!response) {
        throw new Error("Error Post Thread")
      }
      setTweet({
        description: ""
      })
    ...
    return (
        <div className="flex-1">
            {/* <!-- ! Navigation --> */}
            <div className="border-b border-neutral-800 w-full max-h-[14vh]">
                ...
                <div className="flex py-2">
                    ...
                    {navigation.pathname == "/following" ? (
                        <Link to={"/following"} className={active}>
                            Following
                        </Link>
                    ) : (
                        <Link to={"/following"} className={inActive}>
                            Following
                        </Link>
                    )}
                </div>
            ...
            <div className="overflow-auto h-[86vh]">
                <div className="flex border-b border-neutral-800 py-2 pb-3 px-4 gap-4">
                    ...
                    <div className="w-full flex flex-col justify-between gap-2">
                        <form onSubmit={(event) => postTweet(event)}>
                            <div>
                                <input
                                    type="text"
                                    name="description"
